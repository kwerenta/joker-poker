...
int main(int argc, char *argv[]) {
  ...
  while (state.running) {
    ...
    ui_elements[overlay] = (UIElement){.type = UI_ELEMENT_TYPE_RECTANGLE, .rectangle = {.color = RGBA(0, 0, 0, 150)}};
    ...
    for (uint8_t i = 0; i < 12; i++) {
      ...
      ui_elements[level] = (UIElement){.type = UI_ELEMENT_TYPE_RECTANGLE, .rectangle = {.color = RGB(255, 255, 255)}};
