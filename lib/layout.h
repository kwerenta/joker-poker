...
LAY_STATIC_INLINE void lay_get_rect_xywh(const lay_context *ctx, lay_id id, lay_scalar *x, lay_scalar *y,
...
void lay_reset_context(lay_context *ctx) { ctx->count = 0; }
...
static LAY_FORCE_INLINE void lay_append_by_ptr(lay_item_t *LAY_RESTRICT pearlier, lay_id later,
                                               lay_item_t *LAY_RESTRICT plater) {
...
lay_vec4 lay_get_margins(lay_context *ctx, lay_id item) { return lay_get_item(ctx, item)->margins; }
  ...
  while (start_child != LAY_INVALID_ID) {
    ...
    while (child != LAY_INVALID_ID) {
      ...
      if ((flags & LAY_HFILL) == LAY_HFILL) {
        ...
        if ((fflags & LAY_ITEM_HFIXED) != LAY_ITEM_HFIXED) ++squeezed_count;
...
static LAY_FORCE_INLINE void lay_arrange_overlay_squeezed_range(lay_context *ctx, int dim, lay_id start_item,
                                                                lay_id end_item, lay_scalar offset, lay_scalar space) {
  int wdim = dim + 2;
